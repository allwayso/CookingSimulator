...
- 当前开发分支：`feature/mvp-unity-foundation`
...
- 做菜交互仍是按钮驱动，没有食材、锅具、调料贴图交互。
- 当前 UI 是临时调试界面，未进行正式视觉设计。
- 当前没有厨房背景、食材贴图、锅具贴图或状态图片。
- 当前没有学习 / 挑战模式切换。
- 当前没有做菜阶段的步骤进度提示更新，只显示初始提示。
...
1. 清理当前未提交的文档整理变更，建议在单独分支处理。
2. 给 MVP 临时 UI 加入最基础的 2D 厨房背景、食材和锅具贴图。
3. 完善做菜页的步骤提示，使提示随当前动作推进。
4. 增加学习 / 挑战模式切换，但只改变提示显示，不扩展复杂玩法。
5. 加入 `BURNT` 状态和简单火候计时。
6. 增加 Windows `.exe` 构建验证。
7. 在 MVP 稳定后再设计真实 AI API 接入。
